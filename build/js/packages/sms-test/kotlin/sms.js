(function (root, factory) {
  ...
  var println = kotlin_kotlin.$_$.f1;
  var readlnOrNull = kotlin_kotlin.$_$.g1;
  var Default_getInstance = kotlin_kotlin.$_$.q;
  var split = kotlin_kotlin.$_$.e2;
  var collectionSizeOrDefault = kotlin_kotlin.$_$.v;
  var ArrayList_init_$Create$ = kotlin_kotlin.$_$.d;
  var toMutableList = kotlin_kotlin.$_$.c1;
  var objectMeta = kotlin_kotlin.$_$.x1;
  var Exception = kotlin_kotlin.$_$.j2;
  ...
  var getNumberHashCode = kotlin_kotlin.$_$.o1;
  ...
  var isDigit = kotlin_kotlin.$_$.b2;
  var StringBuilder_init_$Create$ = kotlin_kotlin.$_$.h;
  var isLetterOrDigit = kotlin_kotlin.$_$.c2;
  var charSequenceGet = kotlin_kotlin.$_$.i1;
  ...
  var toDouble = kotlin_kotlin.$_$.g2;
  var toBoolean = kotlin_kotlin.$_$.f2;
  var interfaceMeta = kotlin_kotlin.$_$.r1;
  var toSet = kotlin_kotlin.$_$.d1;
  var noWhenBranchMatchedException = kotlin_kotlin.$_$.o2;
  var numberToLong = kotlin_kotlin.$_$.v1;
  var objectCreate = kotlin_kotlin.$_$.w1;
  var numberToInt = kotlin_kotlin.$_$.u1;
  var toString_0 = kotlin_kotlin.$_$.a2;
  var Map = kotlin_kotlin.$_$.u;
  var isInterface = kotlin_kotlin.$_$.t1;
  var isArray = kotlin_kotlin.$_$.s1;
  var List = kotlin_kotlin.$_$.t;
  var Long = kotlin_kotlin.$_$.k2;
  var charSequenceLength = kotlin_kotlin.$_$.j1;
  var getOrNull = kotlin_kotlin.$_$.x;
  var mapCapacity = kotlin_kotlin.$_$.z;
    ...
    switch (expr.get_operator_uy5ppp_k$()) {
      case '+':
        ...
        if (tmp_1) {
          ...
          if (tmp_2) {
            tmp_0 = new StringValue(left.toString() + right.toString());
          } else {
      ...
      case '-':
        ...
        var tmp_4;
        if (left instanceof NumberValue) {
          tmp_4 = right instanceof NumberValue;
        } else {
          tmp_4 = false;
        }

        if (tmp_4) {
          tmp_3 = new NumberValue(left.get_value_j01efc_k$() - right.get_value_j01efc_k$());
        } else {
          throw new RuntimeError("Invalid operands for '-'", expr.get_position_jfponi_k$());
        }
      ...
      case '<':
        ...
        if (tmp_10) {
          tmp_9 = new BooleanValue(left.get_value_j01efc_k$() < right.get_value_j01efc_k$());
        } else {
          throw new RuntimeError("Invalid operands for '<'", expr.get_position_jfponi_k$());
        }
  ...
  function nextToken($this) {
    ...
    var startPos = currentPosition($this);
    var c = advance($this);
    var tmp;
  ...
  function string($this, startPos) {
    var value = StringBuilder_init_$Create$();
    while (!(peek($this) === _Char___init__impl__6a9atx(34)) ? !isAtEnd($this) : false) {
      if (peek($this) === _Char___init__impl__6a9atx(10)) {
        $this.line_1 = $this.line_1 + 1 | 0;
        $this.column_1 = 1;
      } else if (peek($this) === _Char___init__impl__6a9atx(92)) {
        advance($this);
        if (isAtEnd($this)) {
          throw new LexError('Unterminated string literal - missing closing quote', startPos);
        }
        var escaped = advance($this);
        if (escaped === _Char___init__impl__6a9atx(110)) {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(10));
        } else if (escaped === _Char___init__impl__6a9atx(116)) {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(9));
        } else if (escaped === _Char___init__impl__6a9atx(114)) {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(13));
        } else if (escaped === _Char___init__impl__6a9atx(92)) {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(92));
        } else if (escaped === _Char___init__impl__6a9atx(34)) {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(34));
        } else {
          value.append_am5a4z_k$(_Char___init__impl__6a9atx(92));
          value.append_am5a4z_k$(escaped);
        }
      } else {
        value.append_am5a4z_k$(advance($this));
      }
    }
    if (isAtEnd($this)) {
      throw new LexError('Unterminated string literal - missing closing quote', startPos);
    ...
    advance($this);
    return new Token(TokenType_STRING_getInstance(), value.toString(), startPos);
  }
  ...
  function identifier($this, start, startPos) {
    ...
    // Inline function 'kotlin.js.asDynamic' call
    var text = this_0.substring(start, endIndex);
    var tmp0_elvis_lhs = get_KEYWORDS().get_wei43m_k$(text);
  ...
  function skipWhitespace($this) {
    $l$loop: while (!isAtEnd($this)) {
      ...
      if ((tmp0_subject === _Char___init__impl__6a9atx(32) ? true : tmp0_subject === _Char___init__impl__6a9atx(9)) ? true : tmp0_subject === _Char___init__impl__6a9atx(13)) {
        advance($this);
      } else
        break $l$loop;
    }
  ...
  function TokenType_initEntries() {
    ...
    TokenType_BOOLEAN_instance = new TokenType('BOOLEAN', 2);
    TokenType_NULL_instance = new TokenType('NULL', 3);
    TokenType_IDENTIFIER_instance = new TokenType('IDENTIFIER', 4);
    TokenType_VAR_instance = new TokenType('VAR', 5);
    TokenType_FUN_instance = new TokenType('FUN', 6);
    TokenType_IF_instance = new TokenType('IF', 7);
    TokenType_ELSE_instance = new TokenType('ELSE', 8);
    TokenType_WHILE_instance = new TokenType('WHILE', 9);
    TokenType_FOR_instance = new TokenType('FOR', 10);
    TokenType_IN_instance = new TokenType('IN', 11);
    TokenType_BREAK_instance = new TokenType('BREAK', 12);
    TokenType_CONTINUE_instance = new TokenType('CONTINUE', 13);
    TokenType_RETURN_instance = new TokenType('RETURN', 14);
    TokenType_DATA_instance = new TokenType('DATA', 15);
    TokenType_CLASS_instance = new TokenType('CLASS', 16);
    TokenType_PLUS_instance = new TokenType('PLUS', 17);
    TokenType_MINUS_instance = new TokenType('MINUS', 18);
    TokenType_MULTIPLY_instance = new TokenType('MULTIPLY', 19);
    TokenType_DIVIDE_instance = new TokenType('DIVIDE', 20);
    TokenType_ASSIGN_instance = new TokenType('ASSIGN', 21);
    TokenType_EQUALS_instance = new TokenType('EQUALS', 22);
    TokenType_NOT_EQUALS_instance = new TokenType('NOT_EQUALS', 23);
    TokenType_LESS_instance = new TokenType('LESS', 24);
    TokenType_LESS_EQUAL_instance = new TokenType('LESS_EQUAL', 25);
    TokenType_GREATER_instance = new TokenType('GREATER', 26);
    TokenType_GREATER_EQUAL_instance = new TokenType('GREATER_EQUAL', 27);
    TokenType_NOT_instance = new TokenType('NOT', 28);
    TokenType_AND_instance = new TokenType('AND', 29);
    TokenType_OR_instance = new TokenType('OR', 30);
    TokenType_INCREMENT_instance = new TokenType('INCREMENT', 31);
    TokenType_DECREMENT_instance = new TokenType('DECREMENT', 32);
    TokenType_LEFT_PAREN_instance = new TokenType('LEFT_PAREN', 33);
    TokenType_RIGHT_PAREN_instance = new TokenType('RIGHT_PAREN', 34);
    TokenType_LEFT_BRACE_instance = new TokenType('LEFT_BRACE', 35);
    TokenType_RIGHT_BRACE_instance = new TokenType('RIGHT_BRACE', 36);
    TokenType_LEFT_BRACKET_instance = new TokenType('LEFT_BRACKET', 37);
    TokenType_RIGHT_BRACKET_instance = new TokenType('RIGHT_BRACKET', 38);
    TokenType_COMMA_instance = new TokenType('COMMA', 39);
    TokenType_DOT_instance = new TokenType('DOT', 40);
    TokenType_SEMICOLON_instance = new TokenType('SEMICOLON', 41);
    TokenType_NEWLINE_instance = new TokenType('NEWLINE', 42);
    TokenType_EOF_instance = new TokenType('EOF', 43);
    TokenType_UNKNOWN_instance = new TokenType('UNKNOWN', 44);
  }
  var $ENTRIES;
  ...
  function primary($this) {
    ...
    if (match_0($this, [TokenType_BOOLEAN_getInstance()])) {
      ...
      tmp = new NumberLiteral(toDouble(previous($this).get_text_wouvsm_k$()), previous($this).get_position_jfponi_k$());
      ...
      tmp = new ArrayLiteral(elements, previous($this).get_position_jfponi_k$());
    } else {
  ...
  function consume($this, type, message) {
    ...
    throw new ParseError(message, peek_0($this).get_position_jfponi_k$());
  }
  ...
  function NativeFunctionRegistry$registerBuiltins$lambda_9(args) {
    // Inline function 'kotlin.collections.isNotEmpty' call
    if (!args.isEmpty_y1axqb_k$()) {
      println(ValueUtils_getInstance().toString_k2f3al_k$(args.get_c1px32_k$(0)));
    }
    return NullValue_getInstance();
  }
  ...
  _.$_$.d = ScriptEngine;
  _.$_$.e = ScriptError;
  _.$_$.f = NumberValue_init_$Create$;
  _.$_$.g = SMS_getInstance;
